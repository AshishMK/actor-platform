...
public class ShutterButton extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        shadowDrawable.setBounds(cx - Screen.dp(36), cy - Screen.dp(36), cx + Screen.dp(36), cy + Screen.dp(36));
        shadowDrawable.draw(canvas);
        if (pressed || getScaleX() != 1.0f) {
            float scale = (getScaleX() - 1.0f) / 0.06f;
            whitePaint.setAlpha((int) (255 * scale));
            canvas.drawCircle(cx, cy, Screen.dp(26), whitePaint);
            ...
            if (state == State.RECORDING) {
                ...
                canvas.drawCircle(cx, cy, Screen.dp(26) * scale * redProgress, redPaint);
            } else if (redProgress != 0) {
                canvas.drawCircle(cx, cy, Screen.dp(26) * scale, redPaint);
            }
        } else if (redProgress != 0) {
            redProgress = 0;
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        setMeasuredDimension(Screen.dp(84), Screen.dp(84));
    }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        switch (motionEvent.getAction()) {
            case MotionEvent.ACTION_DOWN:
                AndroidUtils.runOnUIThread(longPressed, LONG_PRESS_TIME);
                pressed = true;
                setHighlighted(true);
                break;
            case MotionEvent.ACTION_UP:
                setHighlighted(false);
                AndroidUtils.cancelRunOnUIThread(longPressed);
