...
public class CameraView extends FrameLayout implements TextureView.SurfaceTextureListener {
    ...
    private void initCamera(boolean front) {
        ...
        float screenSize = (float) Math.max(Screen.displaySize.x, Screen.displaySize.y) / Math.min(Screen.displaySize.x, Screen.displaySize.y);
        Size pictureSize;
        ...
        if (textureView.getWidth() > 0 && textureView.getHeight() > 0) {
            int width = Math.min(Screen.displaySize.x, Screen.displaySize.y);
